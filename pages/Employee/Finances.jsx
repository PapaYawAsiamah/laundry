...
const Finances = () => {
  ...
  const makePay = () => {
    setHistory(false)
    setAlertMessage(`confirm payment of GHC${credit} to ${selectedCustomer.label}`);
  ...
  return (
    <>
      ...
      <div style={{ display: "flex" }}>
        ...
        <h2 style={{ marginLeft: 100, color: "red" }}>
          amount left: {history && total - hisTotal}
        </h2>

        {selectedCustomer && (
          <div style={{ display: "flex" }}>
            <input
              style={{ height: 30, marginTop: 30, marginLeft: 100 }}
              type="number"
              onChange={(e) => setCreit(Number(e.target.value))}
            ></input>
            <Button
              variant="contained"
              sx={{ height: 30, marginTop: 4, marginLeft: 2 }}
              onClick={makePay}
            >
              Pay
            </Button>
        ...
        <Button
          variant="contained"
          sx={{ marginLeft: 50, height: 30, marginTop: 4 }}
          onClick={openHistory}
        >
          History
        </Button>
      ...
      {history && (
        <div style={{ height: 500, width: "100%", marginTop: 20 }}>
          ...
          <DataGrid
            ...
            disableSelectionOnClick
          />
          <h2 style={{ color: "green" }}>total: {hisTotal}</h2>
        </div>
      )}
    </>
  );
